...
if(!empty($queryprepared->fetch())){
    ...
    if(!empty($queryprepared->fetch())){
        ...
        $queryprepared = $pdo->prepare("UPDATE utrackpa_users_requests SET statut = 3 WHERE requestedTrack=:id AND requestinguser=:user");
        $queryprepared->execute([
            'id' => $trackId,
            'user' => $userId        
        ]);

        $confirm[] = "Permission allowed successfully";
